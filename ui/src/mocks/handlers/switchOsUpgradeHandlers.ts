...
import { SwitchOsUpgradeWorkflowInput } from "@/types/data-table.types";
import { FORBIDDEN_DEVICE_IDS } from "@/mocks/data/devicesData";

export const switchOsUpgradeHandlers = [
  http.post(
    sanitizeUrl(`${apiURL}/v1/workflow/ngc/switch_os_upgrade`),
    async ({ request }) => {
      const body = (await request.json()) as SwitchOsUpgradeWorkflowInput;
