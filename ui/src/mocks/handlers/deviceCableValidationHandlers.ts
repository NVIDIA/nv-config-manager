...
import { DeviceCableValidationWorkflowInput } from "@/types/data-table.types";
import { FORBIDDEN_DEVICE_IDS } from "@/mocks/data/devicesData";

export const deviceCableValidationHandlers = [
  http.post(
    sanitizeUrl(`${apiURL}/v1/workflow/ngc/device_cable_validation`),
    async ({ request }) => {
      const body = (await request.json()) as DeviceCableValidationWorkflowInput;
