...
import { BackupWorkflowInput } from "@/types/data-table.types";
import { FORBIDDEN_DEVICE_IDS } from "@/mocks/data/devicesData";

export const configBackupHandlers = [
  http.post(
    sanitizeUrl(`${apiURL}/v1/workflow/ngc/backup`),
    async ({ request }) => {
      const body = (await request.json()) as BackupWorkflowInput;
