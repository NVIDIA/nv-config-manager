...
import { FORBIDDEN_DEVICE_IDS } from "../data/devicesData";

export const portLLDPInfoHandlers = [
  http.post(
    sanitizeUrl(`${apiURL}/v1/workflow/ngc/port_lldp_info`),
    async ({ request }) => {
      const body = (await request.json()) as PortLLDPInfoWorkflowInput;
