...
export const cumulusHardwareValidationHandlers = [
  http.post(
    sanitizeUrl(`${apiURL}/v1/workflow/ngc/cumulus_hardware_validation`),
    async ({ request }) => {
      const body =
        (await request.json()) as CumulusHardwareValidationWorkflowInput;
      ...
      if (Object.values(FORBIDDEN_DEVICE_IDS).includes(body.site)) {
        return HttpResponse.json(
          {
