...
// Tests specific to the namespace tag select field introduced in this PR
test.describe("SpX Overlay Creation Workflow Form - Namespace Tag", () => {
  test("namespace_tag URL parameter populates the namespace tag field", async ({
    page,
  }) => {
    // Navigate using the new namespace_tag= URL parameter (not the legacy namespace=)
    await page.goto(
      "/workflows/spxoverlaycreationworkflow/form" +
        `?site=${SITES_LIST.pdx01}` +
        `&overlay_id=${VPC_DATA.overlay_id}` +
    ...
    // Verify the namespace tag field shows the value from the new param
    await expect(
      page.getByRole("button", { name: "tenant-a", exact: true })
    ).toBeVisible({ timeout: TEST_TIMEOUT });
  });

  test("namespace tag select displays available options from API", async ({
    page,
  }) => {
    // Navigate with a site pre-selected so namespace tags are loaded
    await page.goto(
      "/workflows/spxoverlaycreationworkflow/form" +
        `?site=${SITES_LIST.pdx01}` +
        `&namespace=${VPC_DATA.namespace_tag}`
    ...
    // Open the namespace tag dropdown
    await page
      .getByRole("button", { name: VPC_DATA.namespace_tag, exact: true })
      .click();

    // Verify that options from the API mock are visible in the dropdown
    const dialog = page.getByRole("dialog");
    await expect(dialog.getByText("spectrumx")).toBeVisible({
      timeout: TEST_TIMEOUT,
    });
    await expect(dialog.getByText("tenant-a")).toBeVisible({
      timeout: TEST_TIMEOUT,
    });
  });
});
