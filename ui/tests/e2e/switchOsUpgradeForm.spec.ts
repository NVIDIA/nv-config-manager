...
// Add additional tests specific to the switch OS upgrade workflow
test.describe("Switch OS Upgrade Form - Additional Tests", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/workflows/switchosupgradeworkflow/form");
  });

  test("submits correct data to the API", async ({ page }) => {
    // Set up a listener for the request
    const requestPromise = page.waitForRequest((request) => {
      return request.url().includes("/v1/workflow/ngc/switch_os_upgrade");
    });
  ...
  test("loads form with URL parameters and performs manual changes", async ({
    ...
    await page.goto(
      `/workflows/switchosupgradeworkflow/form?site=${siteName}&device-id=${deviceId}`
    );
