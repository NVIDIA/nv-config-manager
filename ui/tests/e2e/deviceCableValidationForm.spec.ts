...
runWorkflowFormTests({
  formPath: "/workflows/devicecablevalidationworkflow/form",
  formTitle: "New Device Cable Validation Workflow",
  defaultPlatform: "Arista EOS", // Assuming this works with Arista devices
});

// Add additional tests specific to the device cable validation workflow
test.describe("Device Cable Validation Form - Additional Tests", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/workflows/devicecablevalidationworkflow/form");
  });

  test("submits correct data to the API", async ({ page }) => {
    // Set up a listener for the request
    const requestPromise = page.waitForRequest((request) => {
      return request.url().includes("/v1/workflow/ngc/device_cable_validation");
    });
    ...
    await page
      .getByRole("heading", { name: "New Device Cable Validation Workflow" })
      .click();
  ...
  test("loads form with URL parameters and performs manual changes", async ({
    ...
    await page.goto(
      `/workflows/devicecablevalidationworkflow/form?site=${siteName}&device-id=${deviceId}`
    );
