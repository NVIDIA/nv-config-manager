...
// Add additional tests specific to the connected host metadata workflow
test.describe("Connected Host Metadata Form - Additional Tests", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/workflows/connectedhostmetadataworkflow/form");
  });

  test("submits correct data to the API", async ({ page }) => {
    // Set up a listener for the request
    const requestPromise = page.waitForRequest((request) => {
      return request.url().includes("/v1/workflow/ngc/connected_host_metadata");
    });
  ...
  test("loads form with URL parameters and performs manual changes", async ({
    ...
    await page.goto(
      `/workflows/connectedhostmetadataworkflow/form?site=${siteName}&device-id=${deviceId}`
    );
