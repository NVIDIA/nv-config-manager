...
test.describe("Deploy Config Form - Additional Tests", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/workflows/deployworkflow/form");
  });

  test("submits correct data to the API", async ({ page }) => {
    // Set up a listener for the request
    const requestPromise = page.waitForRequest((request) => {
      return request.url().includes("/v1/workflow/ngc/deploy");
    });
    ...
    await page
      .getByRole("heading", { name: "New Configuration Deploy Workflow" })
      .click();
  ...
  test("loads form with URL parameters and performs manual changes", async ({
    ...
    await page.goto(
      `/workflows/deployworkflow/form?site=${siteName}&device-id=${deviceId}`
    );
