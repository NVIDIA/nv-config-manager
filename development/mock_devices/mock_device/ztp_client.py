...
def _find_reservation(
    dhcp4: dict, device: DeviceConfig
) -> tuple[dict | None, str]:
    ...
    client_id_colon = (
        ":".join(f"{b:02x}" for b in device.client_id) if device.client_id else ""
    )
...
def _step_dhcp_reservation(
    ...
    try:
        resp = requests.get(
            f"{dhcp_api_url}/config", params={"ip_version": 4}, timeout=30
        )
        resp.raise_for_status()
    except requests.RequestException as exc:
        return ZtpStepResult(
            step="dhcp-reservation", success=False,
    ...
    return ZtpStepResult(
        step="dhcp-reservation", success=True,
...
def _step_fetch_boot_script(
    result: ZtpValidationResult, ztp_api_url: str
) -> ZtpStepResult:
    """Step 2: Fetch the boot script from the ZTP service."""
    if not result.device_uuid:
        return ZtpStepResult(
            step="fetch-boot-script", success=False,
    ...
    content = resp.text
    return ZtpStepResult(
        step="fetch-boot-script", success=True,
...
def _step_validate_serial(
    ...
    try:
        ...
        return ZtpStepResult(
            step="validate-serial", success=False,
            message=f"Failed to reach ZTP service: {exc}",
        )
...
def _step_fetch_config(
    ...
    content = resp.text
    return ZtpStepResult(
        step="fetch-config", success=True,
