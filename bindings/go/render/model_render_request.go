/*
Render API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 0.1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the RenderRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &RenderRequest{}

// RenderRequest Request body for a render operation.
type RenderRequest struct {
	CommitMessage NullableString `json:"commit_message,omitempty"`
}

// NewRenderRequest instantiates a new RenderRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewRenderRequest() *RenderRequest {
	this := RenderRequest{}
	return &this
}

// NewRenderRequestWithDefaults instantiates a new RenderRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewRenderRequestWithDefaults() *RenderRequest {
	this := RenderRequest{}
	return &this
}

// GetCommitMessage returns the CommitMessage field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *RenderRequest) GetCommitMessage() string {
	if o == nil || IsNil(o.CommitMessage.Get()) {
		var ret string
		return ret
	}
	return *o.CommitMessage.Get()
}

// GetCommitMessageOk returns a tuple with the CommitMessage field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *RenderRequest) GetCommitMessageOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.CommitMessage.Get(), o.CommitMessage.IsSet()
}

// HasCommitMessage returns a boolean if a field has been set.
func (o *RenderRequest) HasCommitMessage() bool {
	if o != nil && o.CommitMessage.IsSet() {
		return true
	}

	return false
}

// SetCommitMessage gets a reference to the given NullableString and assigns it to the CommitMessage field.
func (o *RenderRequest) SetCommitMessage(v string) {
	o.CommitMessage.Set(&v)
}

// SetCommitMessageNil sets the value for CommitMessage to be an explicit nil
func (o *RenderRequest) SetCommitMessageNil() {
	o.CommitMessage.Set(nil)
}

// UnsetCommitMessage ensures that no value is present for CommitMessage, not even an explicit nil
func (o *RenderRequest) UnsetCommitMessage() {
	o.CommitMessage.Unset()
}

func (o RenderRequest) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o RenderRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if o.CommitMessage.IsSet() {
		toSerialize["commit_message"] = o.CommitMessage.Get()
	}
	return toSerialize, nil
}

type NullableRenderRequest struct {
	value *RenderRequest
	isSet bool
}

func (v NullableRenderRequest) Get() *RenderRequest {
	return v.value
}

func (v *NullableRenderRequest) Set(val *RenderRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableRenderRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableRenderRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableRenderRequest(val *RenderRequest) *NullableRenderRequest {
	return &NullableRenderRequest{value: val, isSet: true}
}

func (v NullableRenderRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableRenderRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
