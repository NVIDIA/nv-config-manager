...
[**RbacConfigStatusRbacConfigStatusGet**](DefaultAPI.md#RbacConfigStatusRbacConfigStatusGet) | **Get** /rbac-config-status | RBAC Config Status
...
## RbacConfigStatusRbacConfigStatusGet

> RBACConfigResponse RbacConfigStatusRbacConfigStatusGet(ctx).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.DefaultAPI.RbacConfigStatusRbacConfigStatusGet(context.Background()).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `DefaultAPI.RbacConfigStatusRbacConfigStatusGet``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `RbacConfigStatusRbacConfigStatusGet`: RBACConfigResponse
	fmt.Fprintf(os.Stdout, "Response from `DefaultAPI.RbacConfigStatusRbacConfigStatusGet`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiRbacConfigStatusRbacConfigStatusGetRequest struct via the builder pattern


### Return type

[**RBACConfigResponse**](RBACConfigResponse.md)
