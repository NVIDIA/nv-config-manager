...
1. **Version**: Extracted from device's config context `intended-firmware.version`
...
| `desired_firmware` | device_data | string | Target firmware version | `device.config_context['intended-firmware']['version']` |
...
| `local_asn` | device_data | string | Local ASN for Azure devices | `device.config_context['bgp']['local-asn']` |
...
| `site_asn` | location_data | string | Site-level BGP ASN | `location.config_contexts[0].data.site_asn` |
