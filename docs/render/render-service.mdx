...
**Template updater**: Runs as a Kubernetes job on service deployment. It queries Nautobot for devices with stale `template_version` and renders them directly with bounded concurrency and per-device locking.

**Version tracking**: Successful renders record the `nv_config_manager_templates` version in Nautobot so subsequent updater runs can identify stale configurations.

## Deployment

The service is deployed with two consumer deployments (Nautobot and device), a template-updater job that runs on Helm upgrade, and Redis for distributed locking. The service exposes Prometheus metrics on port 8000.
