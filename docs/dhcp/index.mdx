...
The Config Manager splash page summarizes active leases, reservations, pool utilization, and the age of the last successful configuration refresh reported by the DHCP API's `/metrics` endpoint. Active leases are loaded in bounded pages so the dashboard remains responsive with thousands of leases. Use the filter to search the full lease inventory by IP address, hostname, MAC address, client ID, or subnet prefix, and use **Load more leases** to retrieve the next page. Select the **Active leases**, **Reservations**, or **Pool usage** tab to inspect each view. You can also clear an active lease after confirming the action.

![DHCP lease activity dashboard](../assets/images/dhcp/lease-dashboard.png)

The authenticated DHCP API exposes domain-oriented lease resources without leaking the underlying DHCP server's command format. The optional `ip_version` parameter defaults to `4`; pass `ip_version=6` to select IPv6.

List leases with a bounded collection request:

```bash
curl --fail-with-body --get "${DHCP_API_URL}/leases" \
  --header "Authorization: Bearer ${TOKEN}" \
  --data-urlencode "limit=100"
```

The response contains a `leases` array and an opaque `next_cursor`. When `next_cursor` is not null, pass it unchanged in the next request together with the same filters. This also applies to the optional `search` parameter: each request scans a bounded chunk of the lease inventory, and following `next_cursor` continues the same search through the next chunk. The dashboard handles this with **Load more leases**.
...
curl --fail-with-body --get "${DHCP_API_URL}/lease" \
  --header "Authorization: Bearer ${TOKEN}" \
  --data-urlencode "ip_address=7.245.196.5"
```

Delete a lease so the address can be reassigned:

```bash
curl --fail-with-body --request DELETE "${DHCP_API_URL}/lease?ip_address=7.245.196.5" \
  --header "Authorization: Bearer ${TOKEN}"
```

Lease responses use the Config Manager schema and include subnet prefixes instead of DHCP-server-specific subnet identifiers. A successful delete returns `204 No Content`; a missing lease returns `404 Not Found`.
