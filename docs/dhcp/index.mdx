...
The dedicated DHCP dashboard summarizes active leases, reservations, pool utilization, and the age of the last successful configuration refresh reported by the DHCP API's `/metrics` endpoint. Leases, reservations, and pools load 100 at a time as you scroll so the dashboard remains responsive with thousands of records. Use the filter to search the selected collection by its displayed fields. Lease results show the number loaded, with a plus sign while more results are available; reservation and pool results show exact filtered totals. Pool assignment and utilization count only active lease addresses inside the displayed dynamic pool, excluding out-of-pool reservations. Select the **Active leases**, **Reservations**, or **Pool usage** tab to inspect each view. You can also clear an active lease after confirming the action.

![DHCP lease activity dashboard](../assets/images/dhcp/lease-dashboard.png)

Collection requests default `ip_version` to `4`; pass `ip_version=6` to select IPv6. Lease item requests infer the version from the address and also accept an optional matching `ip_version`.
...
Reservations and pool usage use the same bounded query parameters:

```bash
curl --fail-with-body --get "${DHCP_API_URL}/reservations" \
  --header "Authorization: Bearer ${TOKEN}" \
  --data-urlencode "limit=100" \
  --data-urlencode "search=leaf"

curl --fail-with-body --get "${DHCP_API_URL}/pools" \
  --header "Authorization: Bearer ${TOKEN}" \
  --data-urlencode "limit=100"
```

These responses contain `reservations` or `pools`, an exact `total_count` for the current filter, and an opaque `next_cursor`. Pass the cursor unchanged with the same `ip_version`, `limit`, and `search` values to continue.
