...
The Config Manager splash page summarizes active leases, reservations, pool utilization, and the age of the last successful configuration refresh reported by the DHCP API's `/metrics` endpoint. Use the filter to find displayed records by IP address, hostname, identifier, or subnet prefix, and select the **Active leases**, **Reservations**, or **Pool usage** tab to inspect each view. You can also clear an active lease after confirming the action.

![DHCP lease activity dashboard](../assets/images/dhcp/lease-dashboard.png)

The authenticated DHCP API exposes domain-oriented lease resources without leaking the underlying DHCP server's command format. Pass `ip_version=4` or `ip_version=6` to select the address family.
...
```bash
curl --fail-with-body --request DELETE "${DHCP_API_URL}/lease?ip_address=7.245.196.5&ip_version=4" \
  --header "Authorization: Bearer ${TOKEN}"
```
