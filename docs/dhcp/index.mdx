...
The response contains a `leases` array and an opaque `next_cursor`. When `next_cursor` is not null, pass it unchanged in the next request together with the same filters. The optional `search` parameter filters across the complete lease inventory rather than only the current page.
