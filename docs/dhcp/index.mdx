...
The authenticated DHCP API exposes `POST /lease` for looking up and deleting IPv4 leases. The request uses the Kea command envelope, but only the `lease4-get` and `lease4-del` commands are accepted.
