...
These responses contain `reservations` or `pools`, an exact `total_count` for the current filter, and an opaque `next_cursor`. The optional pool `subnet` parameter matches one exact configured prefix. Pass the cursor unchanged with the same `ip_version`, `limit`, `search`, and `subnet` values to continue.
