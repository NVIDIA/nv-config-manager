...
The Config Manager splash page summarizes active leases, reservations, pool utilization, and the age of the last successful configuration refresh reported by the DHCP API's `/metrics` endpoint. Active leases are loaded in bounded pages so the dashboard remains responsive with thousands of leases. Use the filter to search the full lease inventory by IP address, hostname, identifier, or subnet prefix, and use **Load more leases** to retrieve the next page. Select the **Active leases**, **Reservations**, or **Pool usage** tab to inspect each view. You can also clear an active lease after confirming the action.
