...
The Config Manager splash page summarizes active leases, reservations, pool utilization, and the age of the last successful IPv4 configuration refresh reported by the DHCP API's `/metrics` endpoint. Use the filter to find displayed records by IP address, hostname, identifier, or subnet, and select the **Active leases**, **Reservations**, or **Pool usage** tab to inspect each view. You can also clear an active lease after confirming the action.

![DHCP lease activity dashboard](../assets/images/dhcp/lease-dashboard.png)

The authenticated DHCP API exposes `GET /lease-dashboard` for the bounded dashboard data and `POST /lease` for looking up and deleting individual IPv4 leases. The command request uses the Kea envelope, but only the `lease4-get` and `lease4-del` commands are accepted.

Look up the device holding a lease:

```bash
curl --fail-with-body --request POST "${DHCP_API_URL}/lease" \
...
The API returns Kea's response unchanged. A successful command has `result: 0`; other result codes and their accompanying text come directly from Kea.
