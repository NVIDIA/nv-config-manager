...
1. **`validate_configuration` — Validate the intended configuration.**

   This stage fetches the device record from Nautobot, loads the latest intended configuration, and builds a candidate diff on the live switch. This confirms that the rendered configuration exists and that the device can parse it without applying any changes. If validation fails, the stage stops before factory reset, links to the invalid intended configuration, and can be retried after the configuration is fixed.
...
- **Workflow status** shows all three stages green on the Config Manager run page, reports that the intended configuration was validated and ZTP completed successfully, and links to the completed backup workflow.
...
- **Backup record** is present in the Config Manager backup store with a timestamp matching the workflow run. If the backup child workflow failed but ZTP succeeded, the device is provisioned but you should re-run the backup workflow directly — see [Configuration Backup](../configuration-backup/index.mdx).
...
Reprovision stops before factory reset and links to the intended configuration that the switch could not load as a candidate. Open the link from the failed stage, fix and re-render the configuration, then retry the stage.
...
- [Configuration Backup](../configuration-backup/index.mdx) — the child workflow that runs at the end of Reprovision.
