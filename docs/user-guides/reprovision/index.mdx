...
1. **`pre_reprovision_backup` — Back up the device and validate the intended configuration.**
   ...
   - If the backup succeeds and detects configuration drift, the stage reports the drift and continues to ZTP.
   - If the backup succeeds without drift, the stage reports that no drift was detected and continues to ZTP.
...
- **Workflow status** shows all three stages green on the Config Manager run page, reports the pre-reprovision backup and drift result, confirms that ZTP completed successfully, and links to both completed backup workflows.
