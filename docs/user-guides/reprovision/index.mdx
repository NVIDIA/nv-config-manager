...
1. **`execute_ztp` — Reset the device and run ZTP.**
   ...
   - Loads the latest intended configuration and builds a candidate diff on the live switch. This validates that the rendered configuration exists and that the device can parse it without applying any changes. If validation fails, the stage stops before factory reset.
...
- **Workflow status** shows both stages green on the Config Manager run page, and the run reports `display: "ZTP completed successfully"` and links to the completed backup workflow.
