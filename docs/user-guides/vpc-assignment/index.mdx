...
The SpX Overlay Assignment workflow binds an existing VPC's VRF to a specific device and to a specific set of ports on that device. It updates Nautobot's intent — the model of which VRF terminates on which port — without touching the device itself; the device-side change happens when a subsequent deploy applies the resulting intended configuration. Assignment is the second step in the SpX Overlay lifecycle, after [SpX Overlay Creation](../vpc-creation/index.mdx) provisions the VRF and Overlay objects.
...
The VPC has not been created yet at this site. Run [SpX Overlay Creation](../vpc-creation/index.mdx) first.
...
- [SpX Overlay Deletion](../vpc-deletion/index.mdx) — tear down a VPC (only after VRFs are unassigned).
