...
1. **`deploy` — Apply the rendered configuration via Tenant Deploy.**
   ...
   Invokes [Tenant Deploy](../tenant-deploy/index.mdx) as a child workflow against the device. The stage display links to the child while it is running and retains that link on failure. Tenant Deploy performs its own diff validation (allowed `nv set` patterns only) before applying.
