...
- **VPC exists at the site** ([SpX Overlay Creation](../vpc-creation/index.mdx) has been run, with the Overlay and L3 VXLANs created in Nautobot).
...
1. **`deploy` — Apply the rendered configuration via Tenant Deploy.**
   ...
   Invokes [Tenant Deploy](../tenant-deploy/index.mdx) as a child workflow against the device. Tenant Deploy performs its own diff validation (allowed `nv set` patterns only) before applying.
...
Either zero or multiple VRFs were returned for the `vpc_id` + `namespace_tag` combination. Run [SpX Overlay Creation](../vpc-creation/index.mdx) if the VPC does not exist; reconcile duplicates in Nautobot if it does.
