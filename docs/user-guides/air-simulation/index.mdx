...
The TUI writes `~/.nvcm-air-sim.yaml` by default so API keys and generated AIR SSH credentials are not saved inside the Git checkout. Treat the file as sensitive if it contains your NGC API key. To use a different location outside the repository, pass `--config /path/to/config.yaml` or set `NVCM_AIR_CONFIG`.
...
Config Manager runs inside the `oob-mgmt-server` node in AIR. The TUI creates an AIR SSH service to that server and prints copyable commands. The Access panel also shows the generated `nvcm` SSH password under **OOB SSH credentials**.
