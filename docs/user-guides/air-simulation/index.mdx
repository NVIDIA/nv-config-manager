...
| **nv-config-manager Version** | Leave blank (`default`) unless a branch build should be stamped with a specific release version, such as `1.3.0-rc.4`. |
| **Deployment Size** | Under **Advanced**. Use `small` for the public DSX Air trial demo. |

The Git ref selects the source checkout for local image builds. The version field controls the local image tag and package version metadata; when left blank, release-tag refs use that tag and branch refs use the installer default.
