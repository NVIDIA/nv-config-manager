...
The Git ref selects the source checkout for local image builds. The version field controls the local image tag and package version metadata, but it cannot stamp a branch build as a different release. When left blank, release-tag refs use that tag and branch refs use the installer default.
