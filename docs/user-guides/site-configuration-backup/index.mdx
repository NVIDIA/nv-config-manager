...
- **Intended configurations are available** in the [Config Store](../../config-store/index.mdx) when you want drift detection on each child backup. Each child compares running config against the device's rendered intended configuration from the Config Store.
