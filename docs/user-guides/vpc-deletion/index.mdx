...
- **VPC is no longer in use on any device.** Query Nautobot to confirm no interfaces remain bound to the VPC's VRFs. The workflow's `interface_count > 0` guard will refuse to delete any in-use VRF; resolving the bindings up front is easier than retrying after the fact.
...
1. **`delete_spx_overlay` — Delete the VPC's VRFs, VXLANs, and Overlay.**

   Queries Nautobot for the VPC's VRFs by `vpc_id` + `namespace_tag` + `site`. If none exist, the stage short-circuits — a successful no-op for a VPC that was never created or is already gone. For each VRF that does exist:
