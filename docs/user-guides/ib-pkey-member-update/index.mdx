...
- **PKey exists** and the desired Overlay record is linked. The workflow does not lazy-create overlays; use [IB PKey Member Add](../ib-pkey-member-add/index.mdx) once first if the Overlay is missing.
...
After submission, a status page shows the seven execution stages.

## Execution stages

The workflow runs seven stages. `validate_diff` requires approval when removals are present.
