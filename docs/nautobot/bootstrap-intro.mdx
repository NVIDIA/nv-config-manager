...
1. **Create or load the topology** with a Design Builder job, Nautobot API script, CSV import, migration script, or manual UI changes
1. **Verify devices and identifiers** used by DHCP/ZTP, such as serial numbers, MAC addresses, management interfaces, and primary IPs
1. **Verify prefixes and IP addresses** for management networks, loopback ranges, point-to-point links, and service networks
1. **Verify cables** against the planned physical topology
1. **Run cable validation** after devices are reachable to compare the documented topology with LLDP neighbor data
1. **Power on or reprovision devices through DHCP/ZTP** after the Nautobot records and rendered configuration inputs exist
1. **Generate and deploy configurations** using Config Manager's template rendering system and workflows
