...
Device account creation is controlled by each device's provider-neutral intent.
The `password_mappings` block maps an OS username to a base secret key,
rotation suffix, and platform role:

<Note>
Nautobot maps its `config_context` construct to provider-neutral intent. Other providers surface the same
credential-mapping intent through their own normalized provider data, so the
workflow does not depend on a config-context field.
</Note>
