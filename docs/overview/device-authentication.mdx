...
Config Manager uses a dedicated service account to connect to managed devices. The default username is `nv-config-manager`. For production deployments, keep the account password in Vault or OpenBao and expose it to Config Manager through External Secrets Operator. Kubernetes secrets are useful for local development and constrained lab installs, but they are not the preferred production source of truth.

Render uses the service account secret to write the hashed device account into intended configuration. ZTP applies that configuration during bootstrap. Temporal later uses the same secret to authenticate to the device for workflows such as backup, deploy, reprovision, and password rotation.
