...
When building dashboards and alerts, prefer filtering on `category` first. Service names can be more granular (for example, `temporal-api` and `temporal-worker`) and are best used as a secondary filter.
