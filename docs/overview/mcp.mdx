...
When `[dcim] provider = nautobot`, the server also exposes Nautobot-specific inventory search and read-only GraphQL schema tools. Those tools are deliberately omitted for other DCIM providers because Nautobot GraphQL types and REST paths are not portable. If you are using Nautobot for a broad inventory question, let the client explore the schema and then issue targeted read-only queries.
