...
This page is an operational reference for Config Manager log categories and
common warning or error messages. Use it when a dashboard, Loki query, or pod
log stream surfaces a message and you need to map it back to the owning service
and likely subsystem.

The structured log fields most useful for triage are:

| Field | Use |
| :---- | :-- |
| `service` | Service that emitted the log, such as `render`, `dhcp`, `ztp`, `temporal`, or `config-store`. |
