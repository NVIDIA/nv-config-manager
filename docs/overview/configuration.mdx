...
NVIDIA Config Manager stores device configurations in the [Config Store service](../config-store/index.mdx). Config Manager [renders](../render/index.mdx) per-device configurations using version-controlled templates and device data from Nautobot. It then writes the intended configuration to the Config Store service. When a device boots, Config Manager retrieves the intended configuration from the Config Store service and a [Temporal workflow](../temporal/index.mdx) (or [ZTP](../network-ztp/index.mdx) on day zero) applies it to the device.
