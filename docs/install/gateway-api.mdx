...
Only one identity-provider callback is required. It uses `oidc.hostname` when
set and otherwise uses `gateway.baseHostname`:

```text
https://<oidc.hostname-or-gateway.baseHostname>/oauth2/callback
```
...
1. Retain `oidc.hostname` when it is already registered with the identity
   provider. The proxy uses it for the single callback route. If it is unset,
   register `https://<gateway.baseHostname>/oauth2/callback` instead.
