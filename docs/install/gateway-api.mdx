...
configured default request timeout and the longer ZTP upload timeout. It does not
render any `gateway.envoyproxy.io` resources.
...
2. Choose `gateway.create: true` for a dedicated Gateway or configure the shared
   Gateway parent fields above.
3. Remove the old `oidc.oauth2Proxy` values. Ensure the Secret named by
...
4. Register only the base-hostname callback URL shown above.
5. Confirm that the Gateway data-plane namespace is allowed by `networkPolicy`.
