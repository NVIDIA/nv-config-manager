...
The Redis metrics switch stores only the explicit preference. Local
observability automatically enables the exporter and Redis PodMonitor without
changing that preference. With monitoring disabled, explicit Redis metrics run
the exporter without creating a Redis PodMonitor. External Redis suppresses
both generated values.
